...
namespace OnCall.Web.Domain.Repository.Systems.impl
{
    ...
    public class SysMenuRepository : ISysMenuRepository
    {
        ...
        {
            return await _context.QueryAsync<ESysMenu>("select MenuID,MenuTitle,MenuUrl,ParentID,Style,Sort,Status from Sys_Menu");
        }

        public async Task<ESysMenu> GetByIDAsync(int MenuID)
        {
            string sql = "select MenuTitle,MenuUrl,ParentID,Style,Sort,Status from Sys_Menu where MenuID=@MenuID";
